...
Next navigate into the base directory containing the taxonomy:
...
First, stop the Granite model in the **upper** terminal window with the kbd:[CTRL+C] keystrokes:
...
Verify the model has stopped with the following output:

.Output
[source,console,copy=false,subs=attributes+]
----
^C INFO 2025-03-02 16:50:48,806 instructlab.model.serve:213: Server terminated by keyboard
----
...
NOTE: The below command uses a wildcard to find the file as it is the only one present. Outside this lab environment you will need to find the appropriate time stamp.

.Command (Optional)
[source,console,role=execute,subs=attributes+]
----
cat /home/instruct/.local/share/instructlab/datasets/knowledge_train_msgs*.jsonl
----
...
We have provided a model that has already been through this process in your demo system. First, if you have any processes running in either terminal window, type kbd:[CTRL+C] to exit.

To serve the newly trained model run the following in the *upper* command window:
...
Once the model started you will see a similiar output to the following:
