...
[start=2]
. Reference of installing the command line tool using the pip command
...
Now that we know that the command-line interface `ilab` is working correctly, the next step is to initialize the local environment so that we can begin working with the model. This was accomplished by issuing a simple init command.
